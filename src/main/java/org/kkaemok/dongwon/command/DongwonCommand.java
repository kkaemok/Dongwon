...
public final class DongwonCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleSetJob(CommandSender sender, String[] args) {
        if (args.length < 3) {
    ...
    private boolean handleSetCurrency(CommandSender sender, String[] args) {
        if (args.length < 4) {
    ...
    private boolean handleGiveCurrency(CommandSender sender, String[] args) {
        if (args.length < 4) {
    ...
    private boolean handleGuildManage(CommandSender sender, String[] args) {
        if (args.length < 4) {
        ...
        return switch (manageType) {
            case "이름설정" -> {
                guildManager.adminSetGuildName(guildName, args[3]).sendTo(sender);
                yield true;
            }
            case "색설정" -> {
                guildManager.adminSetGuildColor(guildName, args[3]).sendTo(sender);
                yield true;
            }
            case "플레이어설정" -> {
                if (args.length >= 5) {
            ...
            default -> {
                sender.sendMessage("§c길드관리 하위 명령은 이름설정, 색설정, 플레이어설정만 사용 가능합니다.");
                yield true;
            }
        };
    }
    ...
    public @Nullable List<String> onTabComplete(
        ...
        if (args.length == 1) {
            return Arrays.asList("직업설정", "캔설정", "캔지급", "길드관리").stream()
        ...
        if (sub.equals("길드관리")) {
            ...
            if (args.length == 3) {
                return Arrays.asList("이름설정", "색설정", "플레이어설정").stream()
                        .filter(option -> startsWith(option, args[2]))
                        .collect(Collectors.toList());
            }
            if (args.length == 4) {
                if (args[2].equalsIgnoreCase("색설정")) {
                    return Arrays.asList("빨강", "노랑", "연두", "하늘", "보라", "흰색", "회색", "&c", "&e", "&a", "&b", "&d")
                            .stream()
